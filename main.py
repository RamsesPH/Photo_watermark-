...
LIGHTBLUE = '#0078D7'
SKY_BLUE = '#00B7C3'
FONT_1 = ('times', 24, 'bold')
FONT_2 = ('courier', 14, 'bold')
program_is_on = False
...
# add a Title Label
title_label = Label(window, text='Please, Upload Your Image', width=30, font=FONT_1, bg=BACK_COLOR)
title_label.pack(pady=20)

# Todo 3 add the image browsing/loading button.
# Create a button to display the text of entry widget
button = ttk.Button(window, text="Upload Image", command=select_files, padding=10)
button.pack(pady=20)
...
water_label.pack()
texto = Entry(window, font=FONT_2)
texto.pack()

# Todo 5  Add a button for download the final product and a box foe adding the new location for the file.
water_button = ttk.Button(window, text="Apply the Watermark", command=create_watermark, padding=10)
water_button.pack(pady=10)

want_to_save = ttk.Button(window, text="Save Your Photo", command=photo_saver, padding=10)
want_to_save.pack(side=LEFT, anchor=N, expand=True, pady=10, padx=10)

# todo 6 add a button for leaving the app or to watermark another photo.
leave_fmr = tk.Frame(window, relief=tk.RAISED, bd=4)
leave = ttk.Button(leave_fmr, text="QUIT", command=quit)
leave.pack(side=LEFT, anchor=N, expand=True, padx=0, pady=0)
leave_fmr.pack(side=LEFT, anchor=N, expand=True, padx=10, pady=10)

# todo 7 add a button for restarting the program
one_fmr = tk.Frame(window, relief=tk.RAISED, bd=4)
one_more = ttk.Button(one_fmr, text="Restart", command=restart)
one_more.pack(side=LEFT, anchor=N, expand=True)
one_fmr.pack(side=LEFT, anchor=N, expand=True)
